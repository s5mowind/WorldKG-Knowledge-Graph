...
for r in g.query(spatial_object_query):
    row = {'predicate': f"wkg:{r['item'].split('/')[-1]}",
           'label': r['name'],
           'location': r['pos'],
...
spat_obj_df.to_parquet('candidates_type.parquet.gzip', compression='gzip', engine='pyarrow')
...
for relation in pbar:
    ...
    for r in g.query(query_relation % relation.split(':')[-1]):
        row = {'subject': f"wkg:{r['item'].split('/')[-1]}",
               'predicate': relation,
               'literal': r['o'],
               'location': r['pos'],
               'type': r['type'].split('/')[-1] if r['type'] else '<UNK>'}
...
relation_df.to_parquet('subjects_type.parquet.gzip', compression='gzip', engine='pyarrow')
